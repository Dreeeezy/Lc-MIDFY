...
Citizen.CreateThread(function()
	TriggerServerEvent("lc_midfy:GetDate")
end)
...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("lc_midfy:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("lc_midfy:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("lc_midfy:ChangeLoop",nameid,datasoundinfo.loop)
		else
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("lc_midfy:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		TriggerServerEvent("lc_midfy:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("lc_midfy:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("lc_midfy:AddVehicle",vehdata)
			end
		end
	else
...
RegisterNetEvent("lc_midfy:ShowNui")
	AddEventHandler("lc_midfy:ShowNui", function()
	show()
end)
...
AddEventHandler("lc_midfy:AddVehicle", function(data)
...
AddEventHandler("lc_midfy:ChangeState", function(tipo, nome)
...
AddEventHandler("lc_midfy:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
AddEventHandler("lc_midfy:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
RegisterNetEvent("lc_midfy:SendData")
AddEventHandler("lc_midfy:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
			...
			if myjob.name == v.job then
				...
				if distance <= 10 then
					...
					if distance <= 3 then
						...
						if IsControlJustReleased(0, 38) and Check.checkPermissao() then
							nomidaberto = v.name
							show(v.name)
							Wait(1000)
						end
